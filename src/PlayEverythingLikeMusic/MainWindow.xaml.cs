...
namespace PlayEverythingLikeMusic
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void mediaPlayer_Drop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                foreach (string file in files)
                {
                    ...
                    {
                        string path = "output.wav";

                        mediaPlayer.Source = new Uri(file);
                        File.WriteAllBytes(path, Everything2wav(file));
                        //Console.WriteLine(Everything2wav(file).Length);

                        mediaPlayer.Source = new Uri("pack://siteoforigin:,,,/" + path);
                        mediaPlayer.Play();
                        return;
                    }
                }
        ...
        private byte[] Everything2wav(string fileName)
        {
